...
시간 초과가 발생했다 문제에서 낮과 밤이라는 조건이 있어서 같은 좌표를 낮에 방문한 경우와 밤에 방문한경우로 나누었다. 하지만 시간 초과가 발생했다. 
그이유는 연산횟수인거 같은데 
n = 1000, m = 1000, k = 10, 낮밤
처음에 map 을 구성하는데 1,000,000
BFS 를 수행하는데 최악의 경우 모든 갘이
