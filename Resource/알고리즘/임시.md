...
문제 메모리 초과가 발생했다. 에초에 메모리 초과과 발생할 수 밖에 없는 코드 였다 왜냐하면 이미 방문한 숫자여도 더 작은 시간에 방문했다면 모두 더하기 때문에 최악의 경우에 4^500000 숫자가 q 에 삽입될 수 있기 때문이다.  또한 반드시 동일한 시간에 같은 위치에 있어야하는데 이렇게 풀이하면 
